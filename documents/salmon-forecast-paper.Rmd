...
Management of Bristol Bay sockeye salmon is organized at the river-system level. As such forecasts at this resolution are critical for setting management and harvest plans. For each river system, we selected the model with the lowest MASE over the years 2000 to 2019 as the model of choice for that system. Systems varied in both the lowest MASE achieved and in the model that produced the best performance. At least one model was able to beat a simple lag-1 benchmark model in each of the systems, with the `r sys_performers$model[sys_performers$mase == min(sys_performers$mase)]` model achieving a MASE of `r prettyNum(min(sys_performers$mase), figits = 1)` at the top end in the `r sys_performers$system[sys_performers$mase == max(sys_performers$mase)]` system, and the `r sys_performers$model[sys_performers$mase == max(sys_performers$mase)]` model on providing a slight improvement over a lag-1 model with a MASE of `r prettyNum(max(sys_performers$mase), digits = 1)` in the `r sys_performers$system[sys_performers$mase == min(sys_performers$mase)]` river system. Of all the candidate models explored, only the DLM, boosted regression tree, and random forest models were selected as the best performer in at least one system (Fig.\@ref(fig:sys-forecast)). 
...
mean_mase_delta <- system_performance %>% 
    filter(!model %in% c("fri", "boost_tree_ensemble")) %>% 
  group_by(system) %>% 
  mutate(mase_rank = rank(mase)) %>% 
  filter(mase_rank <= 2) %>% 
  arrange(system) %>% 
  group_by(system) %>% 
  summarise(delta  = mase[mase_rank == 2] - mase[mase_rank == 1]) %>% 
  ungroup() %>% 
...
sys_improv <- system_performance %>% 
  ...
  pivot_longer(-c(system, fri), names_to = "model", values_to = "mase") %>% 
  group_by(system) %>% 
  filter(mase == min(mase)) %>% 
  ungroup() %>% 
  summarise(mase_improv = mean((mase - fri) / fri))

age_improv <- age_performance %>% 
  select(model, age_group, mase) %>% 
  pivot_wider(names_from = model, values_from = mase) %>% 
  pivot_longer(-c(age_group, fri), names_to = "model", values_to = "mase") %>% 
  group_by(age_group) %>% 
  filter(mase == min(mase)) %>% 
  ungroup() %>% 
  summarise(mase_improv = mean((mase - fri) / fri))
